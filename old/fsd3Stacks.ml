...
let rec compilarStackO stackA stackB =
  ...
  then begin
    ...
    | 'V'-> print_string "No C-----"; Stack.push (lastPop) stackB; compilarStackO stackA stackB;
    | 'E'->  print_string "No C-----"; Stack.push (lastPop) stackB; compilarStackO stackA stackB;
    | 'C'->  print_string "No C-----"; Stack.push (lastPop) stackB; compilarStackO stackA stackB;
    | 'U'-> 
      let dirs = Stack.pop stackB in
      let esqs = Stack.pop stackB in
      print_string "No U-----";
...
let rec percorrerEpsilon stateS nextStates visited =
  ...
  if(List.length stateS.epsilon > 0) then
    let aux trans =
      ...
      if(not (doesStackContain st visited)) then begin
        Stack.push st visited; 
        percorrerEpsilon st nextStates visited;
      end
    in
    List.iter aux stateS.epsilon;
  else Stack.push stateS nextStates 
...
let search nfa word =
  ...
  let inicio = Stack.copy currentState in
  ...
  let symbolOfWord symbol =
    ...
    Stack.iter (fun x->stateOfCurrentStates x nextStates symbol) currentState;
    if(!hasFailed) then begin
      Stack.clear currentState;
      copiarParaOutraStack inicio currentState;
      hasFailed:=true;
    end
    else begin
      Stack.clear currentState;
  ...
  !hasMatch || doesStackContainEnd currentState 
(*---------------------------------*)
...
let () =
  ...
  if(search resultingNfa strS) then print_string "True" else print_string "False"

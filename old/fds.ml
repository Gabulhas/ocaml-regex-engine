...
let emptyStack outQueue operatorStack=
  let flag = ref false in
  while(Stack.top operatorStack != '(') do
    Queue.push (Stack.pop operatorStack) outQueue;

  done;
    Queue.push (Stack.pop operatorStack ) (Queue.create ());;
...
let getvalueIn a =
  ...
  |_ -> 0
;;
(*let  auxPopper chr outQueue operatorStack=*)


let operator (chr:char) outQueue operatorStack=
    print_string "Char: ";
    print_char chr;
  ...
  if(Stack.is_empty operatorStack) then Stack.push chr operatorStack else
  if(getvalueIn chr > getvalueIn (Stack.top operatorStack) || Stack.top operatorStack = '(' ) then Stack.push chr operatorStack
  
  else
   begin
    let flag = ref true in
    while (not (Stack.is_empty operatorStack) && !flag) do

      if(getvalueIn (Stack.top operatorStack) < getvalueIn chr || Stack.top operatorStack == '(') then
        flag:=false
      else
        Queue.push (Stack.pop operatorStack) outQueue
...
let popper chr outQueue operatorStack=
  print_string "\n--outQueue---> ";
  Queue.iter print_char outQueue;
  print_char '\n';
  print_string "\n##operatorStack###> ";
  Stack.iter print_char operatorStack; 
  print_char '\n';
   print_char chr;
  match chr with
  | '*' -> operator chr outQueue operatorStack
  | '.' -> operator chr outQueue operatorStack
  | '+' -> operator chr outQueue operatorStack
  | '(' -> Stack.push chr operatorStack
  | ')' -> emptyStack outQueue operatorStack
  | ' ' -> ()
  | _ -> Queue.push chr outQueue

let toInfix str=
  let outQueue = Queue.create () in
  let operatorStack = Stack.create () in
  let aux chr =
    popper chr outQueue operatorStack in
  String.iter aux str;
  while (not (Stack.is_empty operatorStack))do
    let lastPoped= Stack.pop operatorStack in
    if(lastPoped != '(') then
    Queue.push lastPoped outQueue;
  done;
  print_string "\n\n\n\n";
  Queue.iter print_char outQueue

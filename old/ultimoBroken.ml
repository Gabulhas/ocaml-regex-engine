...
type transition = {mutable next:state ; mutable symbol:char}
...
let createC (c:char) =
  ...
  let newTrans = {next=  second;symbol=c} in
...
let createE =
  let first= createEstado false in
  let second= createEstado true in
  let newTrans = {next=second; symbol='\n'} in
...
let addEpsilonTransition from destination=
  from.epsilon <- from.epsilon @ [{next = destination;symbol='\n'}];;
...
let search nfa word =
  ...
  let symbolOfWord symbol =
    ...
    if(not !hasMatch) then begin
      ...
      if(!hasFailed) then begin
        ...
        if(not !hasChanged) then currentState:=!nextStates;
        hasChanged:=true;
        hasFailed:=true;
      end
    end
  ...
  (*Printf.printf "size of isEnd list %i" (List.length !currentState);*)
...
let run padrao adn =  
  ...
  let deuMatch = search resultingNfa strS in
  (*Printf.printf "\nMatching Time elapsed %g s\n" (Sys.time() -. matchTime);*)
  (*Printf.printf "\nTotal Time elapsed %g s\n" (Sys.time() -. totalTime);*)
  if(deuMatch) then print_string "YES" else print_string "NO";
